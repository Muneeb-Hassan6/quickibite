        ...
        <div className="k-column">
          ...
          <div className="k-col-body">
            {readyOrders.map((order) => {
              const isDelivery = order.type && order.type.toUpperCase().includes("DELIVERY");
              return (
                <KitchenCard
                  key={order.id}
                  order={order}
                  isReady={true}
                  btnText={isDelivery ? "Wait for Dispatch" : "Complete"}
                  btnClass={isDelivery ? "" : "green"}
                  onNext={() => {
                    if (!isDelivery) {
                      updateStatus(order.id, "completed");
                    } else {
                      toast.info("Delivery orders must be assigned by the Dispatcher.", {
                        duration: 3000,
                        style: { background: "#333", color: "#fff" },
                      });
                    }
