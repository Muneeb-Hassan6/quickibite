...
const POSTerminal = ({ onPlaceOrder }) => {
  ...
  const handleCheckout = async () => {
    ...
    const orderPayload = {
      ...
      total: getGrandTotal(),
      cart: cart,
      paymentMethod: orderType === "Delivery" ? "Cash on Delivery" : "Counter Payment",
      paymentStatus: orderType === "Delivery" ? "Pending" : "Paid"
    };
      ...
      <div className="menu-side">
        ...
        <div className="pos-grid">
          {filteredItems.map((item) => (
            <div
              key={item.id}
              className="pos-card-with-image"
              onClick={() => handleItemClick(item)}
            >
              {item.image_url ? (
                <div className="card-img-wrapper">
                   <img src={`${import.meta.env.VITE_API_BASE}/uploads/${item.image_url}`} alt={item.title} className="card-img" />
                </div>
              ) : (
                <div className="card-img-wrapper empty-img"></div>
              )}
