...
import { FaBrain, FaRoute, FaCamera, FaCheckCircle, FaMoneyBillWave } from "react-icons/fa";
import Swal from 'sweetalert2';

const DeliveryActions = ({ isArrived, aiData, distance, orderStatus, simulateDriving, handlePhotoUpload, deliveryPhoto, completeDelivery, paymentMethod, total }) => {
    const [cashReceived, setCashReceived] = useState(false);

    const handleCashReceived = () => {
        Swal.fire({
            title: "Cash Received?",
            text: `Did you collect Rs. ${total} from the customer?`,
            icon: "question",
        ...
        <div className={`action-box ${isArrived ? 'arrived' : ''}`}>
            ...
            {orderStatus === "photo_captured" && (
                <div className="photo-proof-container">
                    ...
                    {paymentMethod === 'Cash on Delivery' && !cashReceived && (
                        <button className="btn-direct-complete" onClick={handleCashReceived} style={{ marginTop: '15px', backgroundColor: '#f59e0b' }}>
                            <FaMoneyBillWave /> Collect Rs. {total} Cash
                        </button>
                    )}

                    {(!paymentMethod || paymentMethod !== 'Cash on Delivery' || cashReceived) && (
