...
const OrderTracker = () => {
  ...
  const filteredOrders = liveOrders.filter((order) => {
    if (order.status === "Delivered") return false; // Force hide Delivered orders
  ...
  const ProgressStep = ({ status, currentStatus, label, icon, steps }) => {
    ...
    if (isActive) {
      ...
      if (status === "Dispatched") colorClass = "step-dispatched"; // You can add .step-dispatched in css
    }
      ...
      <div className="otp-cards-container">
        {isLoading && liveOrders.length === 0 ? (
          ...
          filteredOrders.map((order) => {
            let isDelivery = order.order_type && order.order_type.toLowerCase() === "delivery";
            let currentStat = order.status;
            if (currentStat === "Completed") currentStat = "Ready";

            let steps = isDelivery ? ["Pending", "Cooking", "Ready", "Dispatched"] : ["Pending", "Cooking", "Ready"];
            
            let currentIndex = steps.indexOf(currentStat);
            if (currentIndex === -1) currentIndex = 0;
            
            // Calculate progress bar width based on steps array length
            let progressPercentage = (currentIndex / (steps.length - 1)) * 100;
            if (progressPercentage === 0) progressPercentage = 15;
            let barWidth = `${progressPercentage}%`;
            
            let barColor = "#f59e0b"; // Yellow (Pending)
            ...
            } else if (currentStat === "Dispatched") {
