...
// ─── RESILIENT TWO-STEP ACCURATE GEOLOCATION GETTER ───
export const getAccuratePosition = (onSuccess, onError) => {
  ...
  const highAccuracyOptions = {
    ...
    timeout: 10000, // 10 seconds for hardware GPS
    maximumAge: 30000,
  };

  const lowAccuracyOptions = {
    enableHighAccuracy: false,
    timeout: 7000, // 7 seconds for cellular / Wi-Fi triangulation
    maximumAge: 60000,
  };
  ...
  navigator.geolocation.getCurrentPosition(
    onSuccess,
    (err) => {
      // If hardware GPS timed out or position unavailable, gracefully retry with network triangulation
      if (err.code === 3 || err.code === 2) {
        console.warn("High accuracy GPS timed out, retrying with network triangulation...");
        navigator.geolocation.getCurrentPosition(
          onSuccess,
          (fallbackErr) => {
            console.warn("Low accuracy fallback failed:", fallbackErr.message);
