        ...
        >
          ...
          {authModalTab !== "forgot" && (
            <div className="space-y-3 mb-5">
              <div className="relative w-full h-[44px] rounded-xl overflow-hidden">
                {/* Genuine Google Identity button iframe rendered invisibly on top to receive native clicks */}
                <div
                  ref={googleBtnContainerRef}
                  className="absolute inset-0 z-20 opacity-0 cursor-pointer overflow-hidden flex items-center justify-center [&>div]:!w-full [&>div>iframe]:!w-full [&>div>iframe]:!h-full [&>div>iframe]:!cursor-pointer"
                ></div>

                {/* Custom Styled Visual Button underneath */}
                <button
                  type="button"
                  onClick={triggerGooglePrompt}
                  disabled={googleLoading}
                  className="w-full h-full py-2.5 px-4 rounded-xl bg-gray-50 dark:bg-neutral-900 hover:bg-gray-100 dark:hover:bg-neutral-800/90 border border-gray-300 dark:border-neutral-700 hover:border-amber-500 dark:hover:border-[#F59E0B] text-zinc-800 dark:text-white font-semibold text-xs flex items-center justify-center gap-3 transition-all duration-200 shadow-xs group pointer-events-none"
                >
                  {googleLoading ? (
